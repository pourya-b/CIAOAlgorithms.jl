    ...
    if iter.γ === nothing 
        if iter.L === nothing
           ...
           for i in 1:N
            ...
            γ = iter.α * R(iter.N) / minimum(iter.L)
            γ = fill(γ, (N,)) # to make it a vector
            println("gamma specified by min{L}")
        end
    else
...
function Base.iterate(
    ...
    while true # first ls
        ...
        ls_rhs =  temp - real(dot(state.sum_tz, state.z)) # bug prone
        ls_rhs *= iter.N * iter.α       
        
        tol = 10^(-6)  * (1 + abs(ls_rhs)) # bug prone
        R(ls_lhs) <= ls_rhs + tol && break  # the ls condition (Table 1, 1b)
        # R(ls_lhs) <= ls_rhs + iter.ls_tol && break  # the ls condition (Table 1, 1b)
        println("ls 1")
    ...
    while true # second ls
        ...
        # R(ls_lhs) <= ls_rhs + iter.ls_tol && break  # the ls condition (Table 1, 3b)
        println("ls 2")
...
# solution(state::FINITO_lbfgs_adaptive_state) = state.test
