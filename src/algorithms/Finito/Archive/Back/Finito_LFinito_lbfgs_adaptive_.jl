...
function Base.iterate(
    ...
    # full z update 
     while true
        ...
        println("ls 1")
        println("f_model_z   : ", f_model_z)
        println("f_model_cnst: ", f_model_cnst)
    ...
    while true
        ...
        state.sum_fv = 0
        for i in 1:iter.N 
            state.sum_fv += iter.F[i](state.res_zbar)
        end
        ...
        state.γ *= 0.8
        # update av
        state.av .-=  ((state.γ - γ_prev) / iter.N) .* state.sum_nabla_fz
        reset!(state.H)
    end   
    ...
    while while_flag
        ...
        for j in state.inds
            for i in state.ind[j]
                ...
                if flag
                    if state.γ < iter.tol_b / iter.N
                        @warn "parameter `γ` became too small at ls 4, ($(state.γ))"
                        return nothing
                    end
                    prox!(state.z, iter.g, state.av, state.γ)

                    fi_x = gradient!(state.∇f_temp, iter.F[i], state.zbar) # update the gradient and ∇f_temp - don't move this outside of the loop! 
                    state.val_fg = iter.F[i](state.z)  ##### change to function value! #####
                    state.z_trial .= state.z .- state.zbar # placeholder for z - x^full

                    fi_model = fi_x + real(dot(state.∇f_temp, state.z_trial)) + (0.5 * iter.α / state.γ) * (norm(state.z_trial)^2)
                    
                    # tol = 10^(-6) * (1 + abs(state.val_fg))
                    tol = eps(R)
                    if R(state.val_fg) + tol > fi_model
                        println("ls 4")
                        state.γ *= 0.8
                        # update av
                        state.av .=  state.temp - ((state.γ - γ_prev) / iter.N) .* state.sum_nabla_fz
                        state.sum_nabla .= state.sum_nabla_fz
                        reset!(state.H)
                        flag = false
                        while_flag = true
                        break
